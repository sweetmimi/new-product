<template>
  <div id="orderInfo" class="m-orderInfo">
    ...
    <Card class="card-gap" v-if="list.length">
      <p slot="title" class="card-head">{{ $t('taskDetail.overdueCondition.title') }}</p>
      <Table border :columns="columns" :loading="loading" :data="list">
...
import { getOverdueList, getOrderInfo } from '@/api'
  ...
  methods: {
    _getList () {
      ...
      getOverdueList({
        ...
        if (+res.status === 0) {
          this.loading = false
          this.list = res.data.overdueList
        }
      })
    },
    _getOrderInfo () {
