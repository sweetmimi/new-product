      ...
      :cancelText="$t('cancelText')">
        ...
        <Form ref="editUserForm" :model="query" class="edit-form" :label-width="120" :rules="editFormRuler">
          ...
          <FormItem :label="`${$t('userManagement.editInfo.task')}:`">
            <CheckboxGroup v-model="query.mission" offset="1">
              <Checkbox v-for="(task, index) in listSelect.userList.mission" :key="index" :label="task.key">{{task.value}}</Checkbox>
            </CheckboxGroup>
          </FormItem>
        </Form>
        ...
        <Col offset="1" span="17">
          <Row>
            <Col
              ...
              :key="index"
              class="edit-value"
              v-for="(task, index) in checkInfo.mission">{{missionMap[task]}}</Col>
          </Row>
        </Col>
      </Row> -->
      <div slot="footer" class="footer">
...
export default {
  ...
  computed: {
    ...
    missionMap: function () {
      let map = {}
      if (this.listSelect.userList && this.listSelect.userList.mission) {
        this.listSelect.userList.mission.forEach(item => {
  ...
  methods: {
    ...
    _toEdit (staffId) {
      ...
      getUserInfo({staffId: staffId}, (res) => {
        this.checkInfo = res.data.staffInfo
        this.query.mission = res.data.staffInfo.mission
        this.query.canUsePhone = res.data.staffInfo.canUsePhone
    ...
    _fetchData (search) {
      ...
      getUserList({
        ...
        if (+res.status === 0) {
          this.totalList = Number(res.data.arrPager.count)
