...
export default class OrderList extends Vue {
  ...
  loanTimeChange(date: any) {
    let date0 = date[0]+" 00:00:00"
    let date1 = date[0]+" 23:59:59"
    this.searchForm.start = date0
    this.searchForm.end = date1
  }
  sun ( date,name){
    let tableData = date
     tableData.map (item=>{
    ...
    getDailyData(data, (res: any) => {
     ...
     let tableData = []
       ...
       this.tableColumns = this.tableDefine1
        ...
        getRatlist(tableData,['loan_pass_rate','jinjian_rate'])
