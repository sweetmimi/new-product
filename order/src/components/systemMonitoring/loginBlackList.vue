...
import {
  ApiloginBlacklist  ,
  ApiloginBlacklistsave ,
  ApiloginBlacklistremove,
} from '@/api/monitor'
export default {
  name: '',
  components: {},
  ...
  methods: {
    ...
    getTableData() {
      this.loading = true
      ApiloginBlacklist  (this.searchForm, (res) => {
        if (res.code == 0) {
          this.loading = false
    ...
    remove(row) {
      this.$Modal.confirm({
        ...
        mobile: 'Hapus daftar hitam, pengguna dapat masuk ke aplikasi secara normal',
        loading: true,
        onOk: () => {
          ApiloginBlacklistremove(
            {
