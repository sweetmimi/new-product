    ...
    <div class="box" v-if="showTable">
      ...
      <Row align="middle">
       ...
       <Row align="middle" v-if="lender==1">
        <Col span="6" class-name="col-style title">{{ $t('approvalDetail.approvalConclusionHistory.insuranceTitle') }}</Col>
        <Col span="18" class-name="col-style" > {{resultHistory.insurance==2? $t('approvalDetail.result.isInsurance'):$t('approvalDetail.result.unInsurance')}}</Col>
...
<script>
import { getConclusionHistory } from '@/api'
export default {
  name: 'resultHistory',
  props: {
    approveId: {
