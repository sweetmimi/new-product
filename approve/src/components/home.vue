  ...
  <div class="myhome">
    ...
    <div class="table">
      <Form inline :label-width="80">
        <FormItem label="Tanggal">
          <DatePicker transfer type="daterange" :options="timeLimitConfig" format="yyyy-MM-dd"
            @on-change="timeChange" :placeholder="`${$t('selectPrefix')}Tanggal`">
          </DatePicker>
        </FormItem>
        <FormItem class="fix-form-btn">
          <Button type="primary" slot="label"  @click="searchData()" :loading="loading.tableLoading">{{ $t("search") }}</Button>

        </FormItem>
         <!-- <FormItem class="fix-form-btn">
            <Button type="success" slot="label"  @click="exportTable()" :loading="loading.export" >Ekspor</Button>
         </FormItem> -->
      </Form>
        <Table
        height="550"
...
import { createva, getva, dashboardShow, withdraw,lenderData } from "@/api/user.js";
export default {
  name: "",
  components: {},
  ...
  methods: {
    ...
    searchData(){
      ...
      lenderData({
        ...
        page:this.page,
        beginTime:this.searchForm.begin_time,
        endTime:this.searchForm.end_time,
      },res=>{
        if(res.status==0){
          this.loading.tableLoading = false;
          this.tableData = res.data.arrLenderAmountList,
    ...
    exportTable(){
      const payload =  {
       ...
       const Url = process.env.NODE_ENV === "production"
          ? '/approve/getlenderamountlist?' : "/test" + '/approve/getlenderamountlist?'
      window.open(window.location.origin + Url + qs.stringify(payload), '_blank')
    },
    withdraw() {
